...
async def generate(request: GenerateRequest, db: Any = Depends(get_db)):
    ...
    full_path = os.path.join(
        base_dir, "..", "assets", "workflows", f"{workflow_id}.json"
    )
    ...
    # 2. Inject inputs into the target nodes
    for role, value in inputs.items():
        # Handle images: e.g. "image1": ["76", "filename.png"]
        if (
            ...
            if node_id in workflow_data:
                workflow_data[node_id].setdefault("inputs", {})["image"] = (
                    filename
                )
            continue

        # Handle non-image primitive inputs matched via (Input:<role>) title tags
        target_node_id = None
    ...
    # 3. Retrieve ComfyUI connection details from database
    comfyui = await db.comfyuisettings.find_first()
    if not comfyui:
        raise HTTPException(
            status_code=500, detail="ComfyUI settings not found in database."
        )
    ...
    # 4. ComfyUI round-trip execution
    async with httpx.AsyncClient(timeout=120.0) as client:
        # Open WebSocket connection before posting prompt to avoid missing events
        async with websockets.connect(f"{comfy_ws}?clientId={client_id}") as ws:
            ...
            # 5. Extract output image metadata from ComfyUI history
            ...
            if not image_info:
                raise HTTPException(
                    status_code=500, detail="No output image produced."
                )

            # 6. Download rendered image bytes from ComfyUI
            params = {
            ...
            # 7. Persist to local assets cache
            ...
            # 8. Return binary stream to frontend
            return Response(
