...
The app ships without system prompt files — you generate your own using any LLM (LM Studio, ChatGPT, Claude, etc.). Each prompt is a plain `.txt` file that instructs the LLM on its role, constraints, and output format for a specific app action.

Here's what each prompt does and what to ask your LLM to generate:

| Prompt File | App Action | What It Does |
|-------------|-----------|--------------|
| `FleshoutStory_SystemPrompt.txt` | Develop Raw Story | Expands a raw story idea into a dense, dialogue-free narrative arc (3 or 5 paragraphs) with sensory-rich locations and character states |
| `Story2Script_SystemPrompt.txt` | Generate Script | Translates a narrative arc into a beat-by-beat screenplay with Action Beats and Dialogue Beats (max 35 chars per line) |
| `ExtractCharactersLocationsProps_SystemPrompt.txt` | Extract Cast | Reads a script and extracts all characters, locations, and hero props with their visual states, outputting structured JSON |
| `ScriptDialogRefiner_SystemPrompt.txt` | Refine Dialog | Polishes dialogue to be character-specific and subtext-driven without altering plot or structure |
| `Script2Shots_SystemPrompt.txt` | Generate Shots | Breaks a script into a shot list optimized for the video model (shot density, framing, frame math, beat mapping) |
| `GeneratePrompt_SystemPrompt.txt` | Generate Prompt | Writes image generation prompts for assets (characters, locations, props) |
| `GenerateVideoPrompt_SystemPrompt.txt` | Generate Video Prompt | Writes video generation prompts for individual shots |
| `Krea2_SystemPrompt.txt` | *(optional)* | Specialized prompt engineer for Krea-2 image model — reverse-engineers references into rich prose prompts |

**How to generate a prompt:**

1. Open your favorite LLM (LM Studio, ChatGPT, Claude, etc.)
2. Describe the role you need. For example, for the story expander:

   > "Write a system prompt for an AI narrative architect. It takes a target duration and a brief plot summary and expands it into a dense, dialogue-free narrative arc of exactly 3 or 5 paragraphs. It must flesh out locations with sensory details, define characters through physical actions and visual traits, and establish geography. Output format: Markdown with a Cast & Environments section followed by the arc paragraphs. No dialogue. No conversational filler."
